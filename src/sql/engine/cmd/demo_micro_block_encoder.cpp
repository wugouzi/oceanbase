...
#include "storage/blocksstable/encoding/ob_icolumn_encoder.h"
...
#include "storage/blocksstable/encoding/ob_dict_encoder.h"
#include "storage/blocksstable/encoding/ob_integer_base_diff_encoder.h"
#include "storage/blocksstable/encoding/ob_string_diff_encoder.h"
#include "storage/blocksstable/encoding/ob_hex_string_encoder.h"
#include "storage/blocksstable/encoding/ob_rle_encoder.h"
#include "storage/blocksstable/encoding/ob_const_encoder.h"
#include "storage/blocksstable/encoding/ob_column_equal_encoder.h"
#include "storage/blocksstable/encoding/ob_encoding_hash_util.h"
#include "storage/blocksstable/encoding/ob_string_prefix_encoder.h"
#include "storage/blocksstable/encoding/ob_inter_column_substring_encoder.h"
...
template<typename T>
T *ObDemoMicroBlockEncoder::alloc_encoder()
{
  T *encoder = NULL;
  int ret = OB_SUCCESS;
...
void ObDemoMicroBlockEncoder::free_encoder(ObIColumnEncoder *encoder)
{
...
template <typename T>
int ObDemoMicroBlockEncoder::try_encoder(ObIColumnEncoder *&encoder, const int64_t column_index)
{
  int ret = OB_SUCCESS;
  encoder = NULL;
  bool suitable = false;
...
ObDemoMicroBlockEncoder::ObDemoMicroBlockEncoder() : ctx_(), header_(NULL),
...
ObDemoMicroBlockEncoder::~ObDemoMicroBlockEncoder()
{
  reset();
}

int64_t ObDemoMicroBlockEncoder::get_data_size() const {
...
int ObDemoMicroBlockEncoder::init(const ObMicroBlockEncodingCtx &ctx)
{
  // can be init twice
  int ret = OB_SUCCESS;
  if (is_inited_) {
...
int ObDemoMicroBlockEncoder::inner_init()
{
  int ret = OB_SUCCESS;
...
void ObDemoMicroBlockEncoder::reset()
{
...
void ObDemoMicroBlockEncoder::reuse()
{
  ObIMicroBlockWriter::reuse();
  // is_inited_
  // ctx_
...
void ObDemoMicroBlockEncoder::dump_diagnose_info() const
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDemoMicroBlockEncoder::init_all_col_values(const ObMicroBlockEncodingCtx &ctx)
{
  int ret = OB_SUCCESS;
...
void ObDemoMicroBlockEncoder::print_micro_block_encoder_status() const
{
...
void ObDemoMicroBlockEncoder::update_estimate_size_limit(const ObMicroBlockEncodingCtx &ctx)
{
  expand_pct_ = DEFAULT_ESTIMATE_REAL_SIZE_PCT;
...
int ObDemoMicroBlockEncoder::try_to_append_row(const int64_t &store_size)
{
  int ret = OB_SUCCESS;
...
int ObDemoMicroBlockEncoder::append_row(const ObDatumRow &row)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDemoMicroBlockEncoder::pivot()
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDemoMicroBlockEncoder::reserve_header(const ObMicroBlockEncodingCtx &ctx)
{
  int ret = OB_SUCCESS;
...
int ObDemoMicroBlockEncoder::store_encoding_meta_and_fix_cols(int64_t &encoding_meta_offset)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDemoMicroBlockEncoder::build_block(char *&buf, int64_t &size)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDemoMicroBlockEncoder::set_row_data_pos(int64_t &fix_data_size)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
OB_INLINE int ObDemoMicroBlockEncoder::store_data(ObIColumnEncoder &e,
...
int ObDemoMicroBlockEncoder::fill_row_data(const int64_t fix_data_size)
{
  int ret = OB_SUCCESS;
...
int ObDemoMicroBlockEncoder::init_column_ctxs()
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDemoMicroBlockEncoder::process_out_row_columns(const ObDatumRow &row)
{
...
int ObDemoMicroBlockEncoder::process_large_row(
...
int ObDemoMicroBlockEncoder::encoder_detection()
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i <ctx_.column_cnt_; ++i) {
      ...
      if (OB_FAIL(try_encoder<ObRawEncoder>(e, i))) {
        LOG_INFO("MMMMM raw encoder failed to try column out row encoder", K(ret));
      ...
      if (col_ctxs_.at(i).is_out_row_column_) {
        ...
        ObIColumnEncoder *e = nullptr;
        // LOG_INFO("MMMMM choose raw encoder", K(i));
        if (OB_FAIL(try_encoder<ObRawEncoder>(e, i))) {
        ...
        // LOG_INFO("MMMMM choose encoder", K(i));
...
int ObDemoMicroBlockEncoder::fast_encoder_detect(
    const int64_t column_idx, const ObColumnEncodingCtx &cc)
{
  int ret = OB_SUCCESS;
  ObIColumnEncoder *e = NULL;
  // LOG_INFO("MMMMM fast detection");
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
    ...
    LOG_INFO("MMMMM specified encoding", K(column_idx), K(ctx_.column_encodings_[column_idx]));
    ...
    // LOG_INFO("MMMMM constant", K(column_idx));
    if (OB_FAIL(try_encoder<ObConstEncoder>(e, column_idx))) {
      LOG_INFO("MMMMM constant fail", K(column_idx));
      LOG_WARN("try encoder failed");
    }
  }
  if (OB_SUCC(ret) && NULL != e) {
    // LOG_DEBUG("MMMMM used encoder (fast)", K(column_idx),
...
int ObDemoMicroBlockEncoder::try_previous_encoder(ObIColumnEncoder *&e,
...
int ObDemoMicroBlockEncoder::try_encoder(ObIColumnEncoder *&e, const int64_t column_index,
...
int ObDemoMicroBlockEncoder::try_previous_encoder(ObIColumnEncoder *&choose,
          ...
          if (OB_FAIL(try_previous_encoder(e, column_index, *prev))) {
            ...
            if (e->calc_size() <= choose->calc_size()) {
              ...
              if (!need_calc || choose->calc_size() <= acceptable_size) {
                try_more = false;
                // LOG_INFO("MMMMM encoder choose previous", K(column_index));
...
template <typename T>
int ObDemoMicroBlockEncoder::try_span_column_encoder(ObIColumnEncoder *&e,
...
int ObDemoMicroBlockEncoder::try_span_column_encoder(ObSpanColumnEncoder *encoder,
    const int64_t column_index, const int64_t ref_column_index,
    bool &suitable)
{
  int ret = OB_SUCCESS;
...
int ObDemoMicroBlockEncoder::choose_encoder(const int64_t column_idx,
                                        ObColumnEncodingCtx &cc)
{
  int ret = OB_SUCCESS;
  ObIColumnEncoder *e = NULL;
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(try_encoder<ObDictEncoder>(e, column_idx))) {
      LOG_INFO("MMMMM try dict encoder fail", K(column_idx));
      ...
      if (cc.detected_encoders_[ObColumnEqualEncoder::type_]) {
        ...
        if (e->calc_size() < choose->calc_size()) {
          ...
          if (choose->calc_size() <= acceptable_size) {
            // LOG_INFO("MMMMM encoder choose column equal", K(column_idx));
            try_more = false;
          }
        } else {
          free_encoder(e);
      ...
      if (is_string_encoding_valid(sc)) {
        if (cc.detected_encoders_[ObInterColSubStrEncoder::type_]) {
          ...
          if (e->calc_size() < choose->calc_size()) {
            ...
            choose = e;
            if (choose->calc_size() <= acceptable_size) {
              // LOG_INFO("MMMMM encoder choose column substring", K(column_idx));
              try_more = false;
            }
          } else {
            free_encoder(e);
    ...
    if (OB_SUCC(ret) && try_more && choose->calc_size() <= acceptable_size) {
      try_more = false;
      // LOG_INFO("MMMMM encoder choose const", K(column_idx));
    }
      ...
      if ((ObIntSC == sc || ObUIntSC == sc)) {
        if (cc.detected_encoders_[ObIntegerBaseDiffEncoder::type_]) {
          ...
          if (size < choose->calc_size()) {
            ...
            if (!try_more) {
              // LOG_INFO("MMMMM encoder choose int diff", K(column_idx));
            }
          } else {
            free_encoder(e);
            e = NULL;
    ...
    if (OB_SUCC(ret)) {
      // LOG_INFO("MMMMM choose encoder", K(choose->get_type()), K(column_idx));
...
void ObDemoMicroBlockEncoder::free_encoders()
{
  int ret = OB_SUCCESS;
  FOREACH(e, encoders_) {
    free_encoder(*e);
...
int ObDemoMicroBlockEncoder::force_raw_encoding(const int64_t column_idx)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
