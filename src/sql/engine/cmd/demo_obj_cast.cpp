...
static int cast_not_support_enum_set(const ObDemoExpectType &expect_type,
                                     ObDemoObjCastParams &params,
                                     const ObObj &in,
                                     ObObj &out)
{
...
static int cast_identity_enum_set(const ObDemoExpectType &expect_type,
                                  ObDemoObjCastParams &params,
                                  const ObObj &in,
                                  ObObj &out)
{
...
static int cast_not_expected_enum_set(const ObDemoExpectType &expect_type,
                                      ObDemoObjCastParams &params,
                                      const ObObj &in,
                                      ObObj &out)
{
...
static int copy_string(const ObDemoObjCastParams &params,
...
ObNumber ObDemoNumberConstValue::MYSQL_MIN[ObNumber::MAX_PRECISION + 1][ObNumber::MAX_SCALE + 1] = {};
ObNumber ObDemoNumberConstValue::MYSQL_MAX[ObNumber::MAX_PRECISION + 1][ObNumber::MAX_SCALE + 1] = {};
ObNumber ObDemoNumberConstValue::MYSQL_CHECK_MIN[ObNumber::MAX_PRECISION + 1][ObNumber::MAX_SCALE + 1] = {};
ObNumber ObDemoNumberConstValue::MYSQL_CHECK_MAX[ObNumber::MAX_PRECISION + 1][ObNumber::MAX_SCALE + 1] = {};
ObNumber ObDemoNumberConstValue::ORACLE_CHECK_MIN[OB_MAX_NUMBER_PRECISION + 1][ObDemoNumberConstValue::MAX_ORACLE_SCALE_SIZE + 1] = {};
ObNumber ObDemoNumberConstValue::ORACLE_CHECK_MAX[OB_MAX_NUMBER_PRECISION + 1][ObDemoNumberConstValue::MAX_ORACLE_SCALE_SIZE + 1] = {};

int ObDemoNumberConstValue::init(ObIAllocator &allocator, const lib::ObMemAttr &attr)
{
  int ret = OB_SUCCESS;
...
int ObDemoHexUtils::unhex(const ObString &text, ObDemoCastCtx &cast_ctx, ObObj &result)
{
  int ret = OB_SUCCESS;
  ObString str_result;
  char *buf = NULL;
...
int ObDemoHexUtils::hex(const ObString &text, ObDemoCastCtx &cast_ctx, ObObj &result)
{
  int ret = OB_SUCCESS;
  ObString str_result;
  char *buf = NULL;
...
int ObDemoHexUtils::hex_for_mysql(const uint64_t uint_val, common::ObDemoCastCtx &cast_ctx, common::ObObj &result)
{
...
int ObDemoHexUtils::rawtohex(const ObObj &text, ObDemoCastCtx &cast_ctx, ObObj &result)
{
  int ret = OB_SUCCESS;
...
int ObDemoHexUtils::hextoraw(const ObObj &text, ObDemoCastCtx &cast_ctx, ObObj &result)
{
  int ret = OB_SUCCESS;
  if (text.is_null()) {
    result.set_null();
...
int ObDemoHexUtils::get_uint(const ObObj &obj, ObDemoCastCtx &cast_ctx, number::ObNumber &out)
{
  int ret = OB_SUCCESS;
...
int ObDemoHexUtils::uint_to_raw(const number::ObNumber &uint_num, ObDemoCastCtx &cast_ctx, ObObj &result)
{
  int ret = OB_SUCCESS;
...
int ObDemoHexUtils::copy_raw(const common::ObObj &obj, common::ObDemoCastCtx &cast_ctx,
    common::ObObj &result)
{
  int ret = OB_SUCCESS;
  char *buf = NULL;
...
//      if (OB_FAIL(ObDemoHexUtils::hextoraw(in, params, out))) {
...
//      if (OB_FAIL(ObDemoHexUtils::rawtohex(in, params, out))) {
...
static int int_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params, const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
...
static int uint_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                    const ObObj &in, ObObj &out);
static int int_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                   const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
...
static int float_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params, const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(ObEnumType != expect_type.get_type())
...
static int float_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params, const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(ObSetType != expect_type.get_type())
      || OB_UNLIKELY(ObFloatTC != in.get_type_class())) {
...
static int double_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params, const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(ObEnumType != expect_type.get_type())
...
static int double_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params, const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(ObSetType != expect_type.get_type())
      || OB_UNLIKELY(ObDoubleTC != in.get_type_class())) {
...
static int number_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj double_val;
...
static int number_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj double_val;
...
static int datetime_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj str_val;
...
static int datetime_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj str_val;
...
static int date_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj str_val;
...
static int date_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj str_val;
...
static int time_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj str_val;
...
static int time_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj str_val;
...
static int year_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj uint_val;
...
static int year_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  ObObj uint_val;
                      ...
                      const ObObj &in, ObObj &out, const ObDemoCastMode cast_mode)
{
  ...
  if (OB_UNLIKELY(ObStringTC != in.get_type_class() && ObTextTC != in.get_type_class()) 
     ...
     ret = OB_ERR_UNEXPECTED;
     LOG_ERROR("invalid input type", K(ret), K(in), K(expect_type));
  } else if (OB_FAIL(ObDemoHexUtils::hextoraw(in, params, out))) {
...
static int bit_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(ObEnumType != expect_type.get_type())
...
static int bit_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params,
                       const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(ObSetType != expect_type.get_type())
...
static int enumset_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params, const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
...
static int enumset_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params, const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
...
static int enumsetinner_enum(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params, const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
...
static int enumsetinner_set(const ObDemoExpectType &expect_type, ObDemoObjCastParams &params, const ObObj &in, ObObj &out)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(ObEnumSetInnerTC != in.get_type_class())
                      ...
                      const ObObj &in, ObObj &out, const ObDemoCastMode cast_mode)
{
  ...
  if (OB_UNLIKELY((ObRawTC != in.get_type_class()) || OB_UNLIKELY(ObStringTC != ob_obj_type_class(expect_type)))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_ERROR("invalid input type", K(ret), K(in), K(expect_type));
  } else if (OB_FAIL(ObDemoHexUtils::rawtohex(in, params, tmp_obj))) {
                        ...
                        const ObObj &in, ObObj &out, const ObDemoCastMode cast_mode)
{
  ...
  if (OB_UNLIKELY((ObRawTC != in.get_type_class())
    ...
    // raw to clob
    if (OB_FAIL(ObDemoHexUtils::rawtohex(in, params, tmp_obj))) {
      LOG_WARN("fail to rawtohex", K(ret), K(in), K(expect_type));
    } else {
  ...
  if (OB_ISNULL(params.allocator_v2_)) {
    ...
    if (obj.is_number_float()) {
      ...
      if (OB_MAX_NUMBER_PRECISION >= precision
          ...
          && number::ObNumber::MAX_SCALE >= scale
          && scale >= number::ObNumber::MIN_SCALE) {
        min_check_num = &(ObDemoNumberConstValue::ORACLE_CHECK_MIN[precision][scale + ObDemoNumberConstValue::MAX_ORACLE_SCALE_DELTA]);
        max_check_num = &(ObDemoNumberConstValue::ORACLE_CHECK_MAX[precision][scale + ObDemoNumberConstValue::MAX_ORACLE_SCALE_DELTA]);
      } else if (ORA_NUMBER_SCALE_UNKNOWN_YET == scale
                 && PRECISION_UNKNOWN_YET == precision) {
        buf_obj.set_number(obj.get_type(), in_val);
        res_obj = &buf_obj;
      ...
      if (OB_UNLIKELY(precision < scale)) {
        ret = OB_ERR_M_BIGGER_THAN_D;
                ...
                && number::ObNumber::MAX_SCALE >= scale
                && scale >= 0) {
        min_check_num = &(ObDemoNumberConstValue::MYSQL_CHECK_MIN[precision][scale]);
        max_check_num = &(ObDemoNumberConstValue::MYSQL_CHECK_MAX[precision][scale]);
        min_num_mysql = &(ObDemoNumberConstValue::MYSQL_MIN[precision][scale]);
        max_num_mysql = &(ObDemoNumberConstValue::MYSQL_MAX[precision][scale]);
      } else {
        ret = OB_INVALID_ARGUMENT;
        LOG_WARN("invalid arguments", K(ret), K(precision), K(scale));
      }
...
int ObDemoObjCaster::to_datetime(const ObObjType expect_type, ObDemoCastCtx &cast_ctx,
    const ObObj &in_obj, ObObj &buf_obj, const ObObj *&res_obj)
{
  int ret = OB_SUCCESS;
...
int ObDemoObjCaster::bool_to_json(const ObObjType expect_type,
                              ObDemoCastCtx &cast_ctx,
                              const ObObj &in_obj,
                              ObObj &buf_obj,
                              const ObObj *&res_obj)
{
  ...
  ObDemoObjCastParams &params = cast_ctx;
  SET_RES_ACCURACY(DEFAULT_PRECISION_FOR_STRING, DEFAULT_SCALE_FOR_TEXT, res_length);
  return ret;
}

int ObDemoObjCaster::enumset_to_json(const ObObjType expect_type,
                                 ObDemoCastCtx &cast_ctx,
                                 const ObObj &in_obj,
                                 ObObj &buf_obj,
                                 const ObObj *&res_obj)
...
int ObDemoObjCaster::to_type(const ObDemoExpectType &expect_type,
                         ObDemoCastCtx &cast_ctx,
                         const ObObj &in_obj,
                         ObObj &out_obj)
{
...
int ObDemoObjCaster::get_zero_value(const ObObjType expect_type, ObCollationType expect_cs_type, ObObj &zero_obj)
{
  int ret = OB_SUCCESS;
...
int ObDemoObjCaster::enumset_to_inner(const ObObjMeta &expect_meta,
 ...
 * @return
 */
int ObDemoObjCaster::is_cast_monotonic(ObObjType t1, ObObjType t2, bool &is_monotonic)
{
  int ret = OB_SUCCESS;
 ...
 */
int ObDemoObjCaster::is_const_consistent(const ObObjMeta &const_mt,
                                     ...
                                     bool &result)
{
  ...
  if (OB_UNLIKELY(ob_is_invalid_obj_tc(tc1) || ob_is_invalid_obj_tc(tc2))) {
    ...
    if (OB_FAIL(ObDemoObjCaster::is_cast_monotonic(column_mt.get_type(),
                                               calc_type,
                                               result))) {
         LOG_WARN("check is cast monotonic failed", K(ret));
    } else if (!result) {
      // false
    } else if (OB_FAIL(ObDemoObjCaster::is_cast_monotonic(calc_type,
                                                      column_mt.get_type(),
                                                      result))) {
      LOG_WARN("check is cast monotonic failed", K(ret));
    } else {
...
int ObDemoObjCaster::is_order_consistent(const ObObjMeta &from,
                                     const ObObjMeta &to,
                                     bool &result)
{
  int ret = OB_SUCCESS;
...
int ObDemoObjCaster::is_injection(const ObObjMeta &from,
                                const ObObjMeta &to,
                                bool &result)
{
  int ret = OB_SUCCESS;
...
/**
 * @brief ObDemoObjCaster::oracle_number_to_char
 * 将number类型转为兼容oracle的字符串
 * 大于40字节时将会被转成科学计数法,输出40个字节的字符串
 * 小于40字节时原样输出
 */
int ObDemoObjCaster::oracle_number_to_char(const number::ObNumber &number_val,
...
int ObDemoObjCaster::can_cast_in_oracle_mode(
...
const bool ObDemoObjCaster::CAST_MONOTONIC[ObMaxTC][ObMaxTC] =
{
  // null
  {
    false,  // null
...
const bool ObDemoObjCaster::ORDER_CONSISTENT[ObMaxTC][ObMaxTC] =
{
  // null
  {
    false,  // null
...
const bool ObDemoObjCaster::ORDER_CONSISTENT_WITH_BOTH_STRING[ObCharset::VALID_COLLATION_TYPES][ObCharset::VALID_COLLATION_TYPES][ObCharset::VALID_COLLATION_TYPES] =
{
...
const bool ObDemoObjCaster::INJECTION[ObMaxTC][ObMaxTC] =
{
  // null
  {
    false,  // null
...
const bool ObDemoObjCaster::INJECTION_WITH_BOTH_STRING[ObCharset::VALID_COLLATION_TYPES][ObCharset::VALID_COLLATION_TYPES][ObCharset::VALID_COLLATION_TYPES] =
{
  //CS_TYPE_UTF8MB4_GENERAL_CI
  {
      //ci    //utf8bin //bin
...
int ObDemoObjEvaluator::is_true(const ObObj &obj, ObDemoCastMode cast_mode, bool &result)
{
  ...
  if (obj.is_null()) {
    ...
    ObDemoCastCtx cast_ctx(&allocator, NULL, cast_mode, CS_TYPE_INVALID);
    ObObj buf_obj;
    const ObObj *res_obj = NULL;
    if (OB_FAIL(ObDemoObjCaster::to_type(ObNumberType, cast_ctx, obj, buf_obj, res_obj))) {
