...
ObDemoMicroBlockBufferHelper::ObDemoMicroBlockBufferHelper()
...
int ObDemoMicroBlockBufferHelper::open(
...
void ObDemoMicroBlockBufferHelper::reset()
{
  data_store_desc_ = nullptr;
  read_info_ = nullptr;
  micro_block_merge_verify_level_ = 0;
...
int ObDemoMicroBlockBufferHelper::compress_encrypt_micro_block(ObMicroBlockDesc &micro_block_desc)
{
  int ret = OB_SUCCESS;
...
int ObDemoMicroBlockBufferHelper::check_micro_block(
...
int ObDemoMicroBlockBufferHelper::check_micro_block_checksum(
    const char *buf,
    const int64_t size,
...
int ObDemoMicroBlockBufferHelper::prepare_micro_block_reader(
    const char *buf,
    const int64_t size,
...
void ObDemoMicroBlockBufferHelper::print_micro_block_row(ObIMicroBlockReader *micro_reader)
{
  int ret = OB_SUCCESS;
...
int ObDemoMicroBlockBufferHelper::dump_micro_block_writer_buffer(const char *buf, const int64_t size)
{
  int ret = OB_SUCCESS;
  ObIMicroBlockReader *micro_reader = NULL;
  if (OB_FAIL(prepare_micro_block_reader(buf, size, micro_reader))) {
...
/**
 * ---------------------------------------------------------ObDemoMacroBlockWriter--------------------------------------------------------------
 */
ObDemoMacroBlockWriter::ObDemoMacroBlockWriter()
  :data_store_desc_(nullptr),
...
void ObDemoMacroBlockWriter::reset()
{
  data_store_desc_ = nullptr;
...
int ObDemoMacroBlockWriter::open(
    ObDataStoreDesc &data_store_desc,
...
int ObDemoMacroBlockWriter::append_row(const ObDatumRow &row, const int64_t split_size)
{
  int ret = OB_SUCCESS;
  const ObDatumRow *row_to_append = &row;
  if (NULL == data_store_desc_) {
    ret = OB_NOT_INIT;
    STORAGE_LOG(WARN, "The ObDemoMacroBlockWriter has not been opened, ", K(ret));
...
int ObDemoMacroBlockWriter::append_macro_block(const ObMacroBlockDesc &macro_desc)
{
  int ret = OB_SUCCESS;
...
int ObDemoMacroBlockWriter::append_micro_block(const ObMicroBlock &micro_block)
{
  int ret = OB_SUCCESS;
...
int ObDemoMacroBlockWriter::check_data_macro_block_need_merge(const ObMacroBlockDesc &macro_desc, bool &need_merge)
{
  int ret = OB_SUCCESS;
  need_merge = false;
...
int ObDemoMacroBlockWriter::check_order(const ObDatumRow &row)
{
  int ret = OB_SUCCESS;
...
int ObDemoMacroBlockWriter::append_index_micro_block(ObMicroBlockDesc &micro_block_desc)
{
...
int ObDemoMacroBlockWriter::build_micro_block()
{
  int ret = OB_SUCCESS;
...
int ObDemoMacroBlockWriter::build_micro_block_desc(
...
int ObDemoMacroBlockWriter::build_micro_block_desc_with_reuse(
    const ObMicroBlock &micro_block,
    ObMicroBlockDesc &micro_block_desc)
{
  int ret = OB_SUCCESS;
...
int ObDemoMacroBlockWriter::build_micro_block_desc_with_rewrite(
    const ObMicroBlock &micro_block,
    ObMicroBlockDesc &micro_block_desc,
...
bool ObDemoMacroBlockWriter::has_wrote_micro_block()
{
  if (has_wrote_micro_block_) {
    has_wrote_micro_block_ = false;
    return true;
  }
  return false;
}
...
{
  int ret = OB_SUCCESS;
  bool has_wrote_micro_block_ = true;
...
int ObDemoMacroBlockWriter::flush_macro_block(ObMacroBlock &macro_block)
{
  int ret = OB_SUCCESS;
...
int ObDemoMacroBlockWriter::flush_reuse_macro_block(const ObDataMacroBlockMeta &macro_meta)
{
  int ret = OB_SUCCESS;
...
int ObDemoMacroBlockWriter::try_switch_macro_block()
{
  int ret = OB_SUCCESS;
...
int ObDemoMacroBlockWriter::check_write_complete(const MacroBlockId &macro_block_id)
{
  int ret = OB_SUCCESS;
...
int ObDemoMacroBlockWriter::wait_io_finish(ObMacroBlockHandle &macro_handle)
{
...
int ObDemoMacroBlockWriter::alloc_block()
{
  int ret = OB_SUCCESS;
  ObMacroBlockHandle &macro_handle = macro_handles_[current_index_];
...
int ObDemoMacroBlockWriter::check_micro_block_need_merge(
...
int ObDemoMacroBlockWriter::merge_micro_block(const ObMicroBlock &micro_block)
{
  int ret = OB_SUCCESS;
  ObIMicroBlockReader *micro_reader = NULL;
  const bool deep_copy_des_meta = false;
...
int ObDemoMacroBlockWriter::save_last_key(const ObDatumRow &row)
{
  int ret = OB_SUCCESS;
  ObDatumRowkey rowkey;
...
int ObDemoMacroBlockWriter::save_last_key(const ObDatumRowkey &last_key)
{
  int ret = OB_SUCCESS;
  last_key_.reset();
...
int ObDemoMacroBlockWriter::calc_micro_column_checksum(const int64_t column_cnt,
...
int ObDemoMacroBlockWriter::build_micro_writer(ObDataStoreDesc *data_store_desc,
                                           ...
                                           const int64_t verify_level)
{
  int ret = OB_SUCCESS;
  void *buf = nullptr;
  ObDemoMicroBlockEncoder *encoding_writer = nullptr;
  ...
  if (data_store_desc->encoding_enabled()) {
    ...
    if (OB_ISNULL(buf = allocator.alloc(sizeof(ObDemoMicroBlockEncoder)))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      STORAGE_LOG(WARN, "fail to alloc memory", K(ret));
    } else if (OB_ISNULL(encoding_writer = new (buf) ObDemoMicroBlockEncoder())) {
      ret = OB_ERR_UNEXPECTED;
  ...
  if (OB_FAIL(ret)) {
    if (OB_NOT_NULL(encoding_writer)) {
      encoding_writer->~ObDemoMicroBlockEncoder();
...
int ObDemoMacroBlockWriter::open_bf_cache_writer(
...
int ObDemoMacroBlockWriter::flush_bf_to_cache(ObMacroBloomFilterCacheWriter &bf_writer, const int32_t row_count)
{
  int ret = OB_SUCCESS;
  ObMacroBlockHandle &macro_handle = macro_handles_[current_index_];
...
void ObDemoMacroBlockWriter::dump_block_and_writer_buffer()
{
...
void ObDemoMacroBlockWriter::dump_micro_block(ObIMicroBlockWriter &micro_writer)
{
  int ret = OB_SUCCESS;
  char *buf = NULL;
  int64_t size = 0;
...
void ObDemoMacroBlockWriter::dump_macro_block(ObMacroBlock &macro_block)
{
